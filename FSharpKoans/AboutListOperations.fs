    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            match xs.Length<0 with// write a function to find the length of a list
            |false -> xs.Length
            |_ ->0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            match xs.Length<=0 with // write a function to reverse a list here.
            |false -> List.rev xs
            | _ -> []
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            match xs.Length<0 with // write a function which adds 1 to each element
