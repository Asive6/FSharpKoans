...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BSc _ -> "!!SCIENCE!!"
            | BPharm -> "Meh, it's OK."
            | BCom (Management, _) | BCom (Economics,_) | BCom(Management, Economics) -> "Money, money, money."
            | BA (_, _) -> "A thinker, eh?"
        randomOpinion (BSc (ComputerScience, __)) |> should equal "Good choice!"
        randomOpinion (BSc (Mathematics, __))|> should equal "!!SCIENCE!!"
